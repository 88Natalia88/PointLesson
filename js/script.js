...
function animation(){
    gsap.registerPlugin(ScrollTrigger); //регистрация плагина
    //Метод .to (от верстки к другой точке)
    /*gsap.to('.promo__text', {
        duration: 1, //скорость
        color: 'red', 
        delay: 1,  //задержка
        xPercent: -50  // смещение по X
    })*/
    //Метод .from (от другой точки к нашей верстки)
    /*gsap.from('.promo__text', {
        duration: 1, 
        color: 'red'
})*/
// Метод .fromTo(от одной какой-то точки к другой какой-то точки минуя нашу верстку)
/*gsap.fromTo('.promo__text', {fromVars}, {duration: 1, toVars})*/
//Метод timeline
